...
- 当前分支：`codex/stage7-screenshot`，只实现视频截图；截取片段已在 `codex/stage7-trim` 独立分支完成，音频提取待独立分支处理
...
- [x] 视频截图：选择单个视频，指定时间点，输出 PNG / JPG，检查输出路径。
- [ ] 音频提取：从视频中提取音频；输出 MP3 / AAC / WAV / FLAC；根据输入媒体是否有音频流给出提示。
- [x] 视频截图接入任务系统，复用 `JobManager`、`jobs-event`、取消、日志和完成状态。
- [x] 视频截图补充后端请求类型和参数构造测试。
- [ ] 截取片段和音频提取在各自独立分支补充请求类型和参数构造测试。

### 输出物
- [ ] `src/features/trim`
- [x] `src/features/screenshot`
- [ ] `src/features/audio`
- [x] 视频截图对应 Rust 命令和参数构造逻辑。
