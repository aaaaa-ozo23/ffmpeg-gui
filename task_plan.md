...
- 建立前端状态模型：
  ...
  - 当前日志。
  - 错误与通知。
- 每个功能模块放入 `src/features/<feature>`，避免一个巨型 App 组件。
...
- 所有首版功能入口可见。
- 切换功能不丢失全局任务队列状态。
- 页面在桌面窗口常见宽度下不出现文字重叠、控件溢出。
- 不出现“前端直接拼命令字符串”的实现入口。
...
### 目标
完成首个真正处理功能：单文件格式转换，覆盖视频转 MP4 和音频转 MP3/WAV/FLAC。
...
- 前端转换表单：
  ...
  - 音频编码：copy / aac / mp3 / flac。
  - 普通模式与高级设置折叠区。
- 后端定义 `ConvertRequest` 结构体。
- 实现转换参数构造：
  - copy/remux 模式。
  - 重编码模式。
  - 输出覆盖策略。
...
- `ffmpeg/presets.rs` 基础预设。

### 验收标准
- 视频转 MP4 成功，输出可播放。
- 音频转 MP3 / WAV / FLAC 成功，输出可播放。
- copy/remux 与重编码模式都可执行。
- 有进度、取消、日志。
- 中文路径和空格路径可用。
...
- 截取片段：
  - 输入开始时间和结束时间。
  - 快速截取：stream copy，速度快但切点可能不完全精确。
  - 精确截取：重编码，切点更准但耗时更长。
  - UI 明确提示两种模式差异。
- 视频截图：
  - 指定时间点。
