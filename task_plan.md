...
## 当前状态
- 规划制定：complete
- 实际开发：阶段 7 in progress
- 当前分支：`codex/stage7-audio-extract`，只实现音频提取；截取片段已在 `codex/stage7-trim` 独立分支完成，视频截图已在 `codex/stage7-screenshot` 独立分支完成
...
### 主要任务
- [ ] 截取片段：输入开始时间和结束时间；快速 stream copy；精确重编码；UI 明确提示两种模式差异。（独立分支 `codex/stage7-trim`）
- [ ] 视频截图：指定时间点，输出 PNG / JPG，输出路径检查。（独立分支 `codex/stage7-screenshot`）
- [x] 音频提取：从视频中提取音频，输出 MP3 / AAC / WAV / FLAC，根据输入媒体是否有音频流给出提示。
- [x] 音频提取接入任务系统，复用 `JobManager`、`jobs-event`、进度、取消和日志。
- [x] 音频提取补充后端请求类型和参数构造测试。
- [ ] 截取片段和视频截图在最终合并阶段统一回到主线验收。

### 输出物
- [ ] `src/features/trim`
- [ ] `src/features/screenshot`
