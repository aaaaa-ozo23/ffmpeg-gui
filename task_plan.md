...
## 当前状态
- 规划制定：complete
- 实际开发：阶段 8 字幕分支 complete，倍速导出 pending
- 当前分支：`codex/stage8-subtitles`，基于 `codex/stage7-full` 新建，已完成字幕封装和硬字幕烧录；倍速导出仍留待后续分支
...
- [x] 视频截图能处理中文路径、空格路径。
- [x] 音频提取能处理中文路径、空格路径。
- [x] 三个独立分支合并后统一验证阶段 7 全量功能。

## 阶段 8：字幕与倍速导出
**状态：in progress（字幕功能完成，倍速待后续分支）**
...
### 主要任务
- 添加字幕：
  - [x] 第一步支持外挂字幕封装到 MKV / MP4。
  - [x] 第二步支持硬字幕烧录。
  - [x] UI 明确区分“封装字幕”和“烧录字幕”。
  - [x] 处理 SRT / ASS 差异、字幕编码和字体路径风险。
- 倍速导出：
  - [ ] 支持 0.5x / 1.25x / 1.5x / 2x。
  - [ ] 明确这是“导出倍速文件”，不是播放器预览倍速。
  - [ ] 处理视频 PTS 与音频 tempo/filter 参数。
- [x] 字幕功能接入任务系统、日志和错误提示。
- [ ] 倍速功能接入任务系统、日志和错误提示。

### 输出物
- [x] `src/features/subtitle`
- [ ] `src/features/speed`
- [x] 字幕参数构造逻辑。
- [ ] 倍速参数构造逻辑。
...
- [x] 字幕失败时 stderr 日志和任务日志能指导用户下一步。
- [ ] 倍速失败日志待倍速分支实现后验证。
