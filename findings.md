...
- 阶段 7 截取功能在 `codex/stage7-trim` 分支先单独落地，不包含视频截图和音频提取。
- 截取 v1 只做单文件音视频片段截取；转换页继续保留阶段 6 的同类批量选择，截取页使用独立单文件选择状态。
- `TrimRequest` 固定字段为 `inputPath`、`outputPath`、`mediaKind`、`outputFormat`、`mode`、`startSec`、`endSec`、`overwrite`、`sourceDurationSec`。
- `JobKind` 新增 `trim`，前端通过 `enqueue_trim_job` 创建任务，任务继续复用现有 `JobManager`、`jobs-event`、进度、取消和日志。
- 截取输出策略为选择输出目录，前端按 `<原文件名>-trimmed.<格式>` 自动生成输出路径。
- 截取支持视频输出 MP4/MKV/MOV/WebM，音频输出 MP3/WAV/FLAC/AAC/M4A/OGG/Opus；图片和未知媒体类型在 UI 与后端校验中拒绝。
- 快速截取模式使用 stream copy，`-ss` 放在 `-i` 前；精确截取模式使用自动重编码，`-ss` 放在 `-i` 后。
- 截取进度按片段时长 `endSec - startSec` 计算，不按源媒体总时长计算。
- 截取 v1 不保留字幕轨；字幕封装和烧录仍留给阶段 8。
...
- 阶段 7 集成分支 `codex/stage7-full` 已从阶段 6 基线开始合并独立功能分支，当前已合入截取和视频截图。
...
| 阶段 7 截取先做单文件 | 符合基础功能边界，批量同类处理留给阶段 9 |
| 截取输出使用目录加自动命名 | 与阶段 6 批量输出体验一致，避免单文件保存对话框引入额外状态 |
| 精确截取只做自动编码 | 避免把截取页扩展成第二个转换页，自定义 codec 留给后续预设或高级设置 |
| 截取不保留字幕轨 | 字幕处理已规划到阶段 8，本轮避免字幕流与容器兼容性扩大范围 |
